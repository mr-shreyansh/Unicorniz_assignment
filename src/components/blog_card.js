...
const BlogCard = ({title,i,tag1,tag2}) => {
    return ( 
        <div className="flex flex-col bg-white shadow-xl my-5 w-11/12 transition-all hover:-translate-y-1 max-h-[400px] max-w-[320px]">
            <img
                            key={i}
               ...
               <p className="text-gray-500 font-light text-xs px-2 mb-1" >+Explore</p>
                <p className="text-gray-500 font-sm text-xs bg-gray-100 rounded-lg px-2 mb-1">{tag1}</p>
                <p className="text-gray-500 font-sm text-xs bg-gray-100 rounded-lg px-2  mb-1">{tag2}</p>
               </div>
        </div>
     );
}
